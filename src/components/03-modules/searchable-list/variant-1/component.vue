<template>
  <div :class="classes">
    ...
    <TextInput id="xyz" name="xyz" label="Search" placeholder="Type to search.." @input="searchContent()" v-model="searchTerm" />

    <Ul v-if="this.endpoint">
      <li v-for="(datum, index) in listData" :key="index + '-' + datum.title" class="list-item" >
        <Accordion :openText="datum.title" :closedText="datum.title" initialState="closed">
          <T2>{{ datum.content }}</T2>
        </Accordion>
      </li>
    </Ul>
...
<script>
  ...
  export default {
    name: 'SearchableList',
    components: {
      Listen, Container, Layout, PageHeader, SearchableList, ProductList, Logo, PrimaryNavigation, Accordion, AppBar, Pane, Form, LoginForm, Legend, Card, CopyBlock, H1, H2, H3,H4, H5, T1, T2, T3,
    ...
    computed: {
      classes() {
        return `searchable-list ${this.classList}`;  
      },
      listData() {
        return this.data 
      }
    ...
    methods: {
      getEndpoint() {
        if(this.endpoint === "bitesize"){
          this.data = this.$store.getters.bitesize
        } else {
          this.data = this.$store.getters.coaches
        }
      },
      searchContent() {
        //console.log("Typed in the search box..  ", this.searchTerm);
        ...
        if(this.searchTerm  && !this.searchTerm == ""){
          ...
          this.data  = this.$store.getters.bitesize
          results =  this.data.filter(o => Object.keys(o).some(k => String(o[k]).toLowerCase().includes(this.searchTerm.toLowerCase())));

        } else {

          results = this.$store.getters.bitesize
    ...
    mounted() {
      this.$store.dispatch('loadData', 'bitesize');
      this.$store.dispatch('loadData', 'coaches');
    },
    created() {
      if(this.endpoint){
        this.getEndpoint()
