    ...
    <Pane variant="page">
        ...
        <div v-if="currentLevel">
            ...
            <div v-for="(question1, index1) in reducedSet" :key="index1" class="level-1">
                ...
                <RadioButton legend="" label="" @change="reduceSet('1', question1.text)" :name="`radios-${index1}`" :options="question1.text" />

                <div v-for="(question2, index2) in question1.level2" :key="index2" class="level-2">

                    <RadioButton legend=""  label="" @change="reduceSet('2', question2.text)" :name="`radios-${index2}`" :options="question2.text" />

                    <div v-for="(question3, index3) in question2.level3" :key="index3" class="level-3">

                        <RadioButton legend=""  label="" @change="reduceSet('3', question3.text)" :name="`radios-${index3}`" :options="question3.text" />

                        <div v-for="(question4, index4) in question3.level4" :key="index4" class="level-4">

                            <RadioButton legend=""  label="" @change="reduceSet('4', question4.text)" :name="`radios-${index4}`" :options="question4.text" />
            ...
            <Button text="Continue" @click="continueDiagnosis()" />
        ...
        <div v-else>
            ...
            <T2>Lorem ipsum dolor ipsum etiet estet..</T2>

            <Button text="Start" @click="beginDiagnosis()" />
...
<script>
  ...
  export default {
    name: 'Diagnostic',
    components: {
        Ul, H2, T2, Checkbox, RadioButton, Link, Pane, Button
    },
    props: {
      classList: {
        type: String,
    ...
    methods: {
        beginDiagnosis() {
            console.log("Called beginDiagnosis");
            this.currentLevel = "level-1";
        },
        reduceSet(level, selectedText) {
            console.log("Called reduceSet");

            this.$options.currentLevel = "level-" + (parseInt(level) + 1)//Add class to display the appropriate level --
            this["selectionLevel" + level] = selectedText;

        },
        continueDiagnosis(){
            console.log("Called continueDiagnosis");
            ...
            if(this.selectionLevel1){
                ...
                this.reducedSet = this.reducedSet.filter((item) => {
                ...
                if(this.selectionLevel2){
                    ...
                    this.reducedSet[0].level2 = this.reducedSet[0].level2.filter((item) => {
                    ...
                    if(this.selectionLevel3){
                        ...
                        this.reducedSet[0].level2[0].level3 = this.reducedSet[0].level2[0].level3.filter((item) => {
                        ...
                        if(this.selectionLevel4){
                            ...
                            this.reducedSet[0].level2[0].level3[0].level4 = this.reducedSet[0].level2[0].level3[0].level4.filter((item) => {
